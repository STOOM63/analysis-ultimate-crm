...
AU.autopilot = (() => {
  ...
  function run(model){
    ...
    if(ctx?.source){
      ...
      add('public-api-sentinel','Clermont API Sentinel contrôlé',apiText,details,!s.apiOk||s.stale?'warning':'positive');
    }
    ...
    const findings=[...(intel?.findings||[]),...(geo?.findings||[])].sort((a,b)=>(priority(b.level)*100+(b.confidence||0))-(priority(a.level)*100+(a.confidence||0)));
