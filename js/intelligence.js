...
AU.intelligence = (() => {
  ...
  function answerQuestion(model, rawQuery) {
    ...
    if (has('pourquoi', 'baisse', 'chute', 'ca baisse', 'ca diminue')) {
    ...
    if (has('zone', 'secteur', 'geograph', 'géograph', 'brezet', 'brézet', 'clermont est', 'travaux', 'acces', 'accès', 'mobilite', 'mobilité')) {
      ...
      return make('Clermont Geo Intelligence', `Couverture géographique des transactions : ${U().percent(g.coverage)}. Le moteur compare chaque zone à sa période précédente ET au reste de la clientèle.`, fs, top);
    }
    ...
    return make('Brief Analysis Ultimate', intel.brief[0] || 'Analyse automatique terminée.', intel.findings.slice(0,7), intel.actions.slice(0,5).map(a=>`Action : ${a.action}`));
  }
