...
AU.ui = (() => {
  ...
  function dashboard(model, root) {
    ...
    const currentStart = U().addDays(ref, -29);
    const previousStart = U().addDays(ref, -59);
    const previousEnd = U().addDays(ref, -30);
    const current = AU.analytics.periodSummary(model.transactions, currentStart, ref);
    const prev = AU.analytics.periodSummary(model.transactions, previousStart, previousEnd);
    ...
    root.innerHTML = viewHeader('Vue générale', `Données du ${U().formatDate(model.range.min)} au ${U().formatDate(model.range.max)} · ${U().integer(model.transactions.length)} transactions`, qualityPill(q.status, q.status === 'certified' ? 'Croisement certifié' : 'Croisement contrôlé')) +
      `<div class="kpi-grid">
        ${kpi('CA TTC · 30 jours', U().money(current.caTTC), deltaText(U().pctChange(current.caTTC, prev.caTTC)) + ' vs 30 j précédents')}
        ${kpi('Tickets · 30 jours', U().integer(current.tickets), deltaText(U().pctChange(current.tickets, prev.tickets)))}
        ${kpi('Panier moyen', U().money(current.avgBasket), deltaText(U().pctChange(current.avgBasket, prev.avgBasket)))}
        ${kpi('Marge · 30 jours', U().money(current.margin), current.marginRate !== null ? `${U().percent(current.marginRate)} du CA HT` : '')}
      ...
      <div class="grid-2">
        <section class="panel"><div class="panel-title"><div><h2>CA quotidien · 90 derniers jours</h2><div class="panel-sub">Courbe calculée sur les jours ayant au moins une transaction.</div></div></div>${sparkCanvas('salesSpark', last90, 'caTTC')}</section>
        <section class="panel"><div class="panel-title"><div><h2>Lecture intelligente</h2><div class="panel-sub">Faits, calculs et estimations restent distingués.</div></div></div><div class="insight-list">${model.insights.slice(0,6).map(x => `<div class="insight ${x.level}"><div></div><div><strong>${U().escapeHtml(x.title)}</strong><p>${U().escapeHtml(x.detail)}</p><span class="quality-tag">${x.quality}</span></div></div>`).join('') || '<div class="empty-mini">Aucun signal notable.</div>'}</div></section>
      </div>
  ...
  function render(view,model,root){currentModel=model;if(!model){return}const views={dashboard,clients:clientsView,alerts:alertsView,fidelity:fidelityView,products:productsView,rayons:rayonsView,stock:stockView,salesforce:salesforceView,calendar:calendarView,associations:associationsView,compare:compareView,quality:qualityView};(views[view]||dashboard)(model,root)}
