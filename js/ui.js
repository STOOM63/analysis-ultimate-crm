...
AU.ui = (() => {
  ...
  function autopilotView(model, root) {
    ...
    root.innerHTML=viewHeader('Pilotage automatique',`Analysis Ultimate exécute seul les contrôles, classe les priorités et prépare les listes d’action · ${U().formatDate(a.generatedAt)}`)+
  ...
  function geoView(model, root) {
    ...
    if(!g){root.innerHTML=viewHeader('Geo Intelligence','Module géographique indisponible.');return;}
    const pressure=g.zones.filter(z=>z.impactScore>=25);
    const ctx=model.contextCorrelation;
    const weather=ctx?.weather;
    const parking=ctx?.parking;
    root.innerHTML=viewHeader('Geo Intelligence · Clermont',`Analyse automatique du CA, des visites, du panier et de la revisite par zone · couverture ${U().percent(g.coverage)}`)+
      `<section class="geo-hero"><div><span class="eyebrow">CLERMONT IMPACT ENGINE</span><h2>${pressure.length?`${pressure.length} zone(s) sous pression`:'Situation géographique stable'}</h2><p>${U().escapeHtml(g.method)}</p></div><div class="geo-big"><strong>${pressure[0]?.impactScore||0}</strong><span>/100 impact max</span></div></section>
      <div class="kpi-grid section-gap">${g.groups.slice(0,6).map(x=>kpi(x.name,U().money(x.current.ca),`${fmtDelta(x.caDelta)} CA · ${fmtDelta(x.visitsDelta)} visites`)).join('')}</div>
      <section class="panel section-gap"><div class="panel-title"><div><h2>Sources publiques & API</h2><div class="panel-sub">Contrôle automatique de la connexion Clermont Métropole, des jeux de données et des fallbacks.</div></div></div>${sourceHealthView(ctx)}${parking?`<div class="context-zone"><strong>Stationnement Métropole · API temps réel</strong><p>${parking.records} parc(s) avec taux exploitable · occupation moyenne ${parking.avgOccupancy.toFixed(1)} % · maximum ${parking.maxOccupancy.toFixed(1)} %${parking.high?` · ${parking.high} parc(s) ≥ 85 %`:''}.</p>${parking.mostOccupied?`<small class="muted">Plus chargé au dernier relevé : ${U().escapeHtml(parking.mostOccupied.name)} (${parking.mostOccupied.occupancy_pct.toFixed(1)} %).</small>`:''}</div>`:''}${ctx?.apiDatasets?`<div class="context-zone"><strong>Auto-discovery Open Data</strong><p>${ctx.apiDatasets.relevant.length} jeu(x) mobilité/travaux/voirie détecté(s) automatiquement dans le catalogue ; ${ctx.apiDatasets.knownOk}/${ctx.apiDatasets.known.length} jeux structurants connus répondent.</p></div>`:''}</section>
      <section class="panel section-gap"><div class="panel-title"><div><h2>Radar automatique des zones</h2><div class="panel-sub">Chaque zone est comparée à sa période précédente et au reste de la clientèle.</div></div></div><div class="data-table-wrap"><table class="data-table"><thead><tr><th>Zone</th><th>Impact</th><th class="numeric">CA</th><th>Évol. CA</th><th>Visites</th><th>Évol. visites</th><th>Driver</th><th>Écart au reste</th><th>Risque clients</th><th>Prévision 7j</th><th>Rupture détectée</th></tr></thead><tbody>${g.zones.map(z=>`<tr><td><strong>${U().escapeHtml(z.name)}</strong><br><span class="muted">${U().escapeHtml(z.worksSector)}</span></td><td><span class="geo-score ${z.impactScore>=70?'critical':z.impactScore>=45?'warning':z.impactScore>=25?'watch':'stable'}">${z.impactScore}/100</span></td><td class="numeric">${U().money(z.current.ca)}</td><td>${fmtDelta(z.caDelta)}</td><td>${z.current.visits}</td><td>${fmtDelta(z.visitsDelta)}</td><td><strong>${U().escapeHtml(z.driver||'—')}</strong><br><span class="muted">visites ${z.trafficEffect>=0?'+':''}${U().money(z.trafficEffect)} · panier ${z.basketEffect>=0?'+':''}${U().money(z.basketEffect)}</span></td><td>${z.excess===null?'—':`${z.excess>=0?'+':''}${(z.excess*100).toFixed(1)} pts`}</td><td>${z.highRisk} élevés<br><span class="muted">${U().money(z.valueAtRisk)}/mois*</span></td><td>${z.forecast7?`${U().money(z.forecast7.ca)}<br><span class="muted">${z.forecast7.visits.toFixed(1)} visites · ${z.forecast7.confidence}%</span>`:'—'}</td><td>${z.changePoint?`${U().formatDate(z.changePoint.date)}<br><span class="muted">${fmtDelta(z.changePoint.delta)}</span>`:'—'}</td></tr>`).join('')}</tbody></table></div></section>
      <div class="grid-2 section-gap"><section class="panel"><div class="panel-title"><h2>Constats automatiques par zone</h2><span class="panel-sub">Explications générées sans sélection manuelle</span></div><div class="intel-finding-list">${(g.findings||[]).map(f=>findingCard({...f,confidenceLabel:f.confidence>=90?'Très forte':f.confidence>=75?'Forte':'Moyenne'},false)).join('')||'<div class="empty-mini">Aucun signal géographique majeur.</div>'}</div></section>
      <section class="panel"><div class="panel-title"><h2>Contexte externe automatique</h2><span class="panel-sub">API + travaux officiels + météo, sans données clients</span></div>${(ctx?.findings||[]).map(f=>`<div class="context-zone"><strong>${U().escapeHtml(f.title)}</strong><p>${U().escapeHtml(f.text)}</p></div>`).join('')}${weather?`<div class="context-zone"><strong>Contrôle météo</strong><p>${weather.days} jours rapprochés · corrélation pluie/visites ${weather.rainVisitsCorrelation===null?'—':weather.rainVisitsCorrelation.toFixed(2)} · pluie/CA ${weather.rainCaCorrelation===null?'—':weather.rainCaCorrelation.toFixed(2)}.</p></div>`:''}${ctx?.matches?.length?ctx.matches.map(m=>`<div class="context-zone"><strong>${U().escapeHtml(m.zone)}</strong><p>${m.works.length} élément(s) travaux correspondant(s) au secteur officiel · ${m.apiCount||0} via API / ${m.pageCount||0} via pages officielles.</p><ul>${m.works.slice(0,6).map(w=>`<li>${U().escapeHtml(w.place||w.sector)} · ${U().escapeHtml(w.text)} <span class="muted">[${w.source_type==='clermont_api'?'API':'OFFICIEL'}]</span></li>`).join('')}</ul></div>`).join(''):`<div class="empty-mini">Aucun événement travaux n’est actuellement rapproché d’une zone commerciale sous pression. Cela ne bloque jamais l’analyse interne.</div>`}</section></div>
      <section class="panel section-gap"><div class="panel-title"><div><h2>Évolution hebdomadaire par zone</h2><div class="panel-sub">CA et visites agrégés par semaine pour faire ressortir les ruptures.</div></div></div><div class="geo-spark-grid">${g.zones.slice(0,6).map((z,i)=>`<article class="geo-spark-card"><div><strong>${U().escapeHtml(z.name)}</strong><span>${z.series.length?`${U().integer(z.series.at(-1).visits)} visites dernière semaine`:''}</span></div>${sparkCanvas(`geoSpark${i}`,z.series,'ca')}</article>`).join('')}</div></section>
      <section class="panel section-gap"><div class="panel-title"><h2>Historique de surveillance automatique</h2><span class="panel-sub">Un snapshot est conservé localement à chaque analyse</span></div>${model.analysisHistory?.length?`<div class="data-table-wrap"><table class="data-table"><thead><tr><th>Analyse</th><th>Données jusqu’au</th><th class="numeric">Transactions</th><th class="numeric">Clients risque élevé</th><th>Zone la plus tendue</th></tr></thead><tbody>${model.analysisHistory.slice(-12).reverse().map(h=>{const z=[...(h.geo||[])].sort((a,b)=>b.impactScore-a.impactScore)[0];return `<tr><td>${U().formatDate(new Date(h.capturedAt))}</td><td>${U().formatDate(new Date(h.dataMax))}</td><td class="numeric">${U().integer(h.transactions)}</td><td class="numeric">${U().integer(h.riskHigh)}</td><td>${z?`${U().escapeHtml(z.name)} · ${z.impactScore}/100`:'—'}</td></tr>`}).join('')}</tbody></table></div>`:'<div class="empty-mini">Le premier snapshot sera créé après cette analyse ; les suivants permettront de suivre l’évolution entre imports.</div>'}</section>
      <div class="panel-sub section-gap">* valeur mensuelle historique estimée, pas un CA perdu certain. « Écart au reste » sert de zone témoin interne et ne prouve pas à lui seul que les travaux sont la cause. Le statut API réduit automatiquement la confiance accordée aux explications externes lorsqu’une source est indisponible ou ancienne.</div>`;
    requestAnimationFrame(()=>g.zones.slice(0,6).forEach((z,i)=>drawSpark(document.getElementById(`geoSpark${i}`),z.series,'ca')));
  }
