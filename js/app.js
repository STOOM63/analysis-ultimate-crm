...
AU.app = (() => {
  ...
  function updateRunButton() {
    ...
    if (ready && !state.model && !state.analysisRunning) {
      ...
      cross.innerHTML = `<div class="cross-icon">↻</div><div><strong>Sources validées</strong><p>Analysis Ultimate lance automatiquement le croisement et le diagnostic autonome.</p></div>`;
  ...
  async function runAnalysis() {
    ...
    try {
      ...
      showModel();
      closeImport();
      AU.ui.toast(`Croisement réussi. ${model.intelligence?.findings?.length || 0} diagnostics autonomes générés.`, 'good');
    } catch (err) {
      console.error(err);
      cross.className = 'cross-status bad';
  ...
  async function init() {
    ...
    if (!AU.xlsxLite || !AU.csv) {
      ...
      $('#fatal').innerHTML = `<strong>Moteur d’import local indisponible.</strong><br>Rechargez la page : les parseurs CSV/XLSX sont inclus directement dans Analysis Ultimate.`;
    }
