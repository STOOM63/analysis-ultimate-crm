...
AU.app = (() => {
  ...
  function updateRunButton() {
    ...
    if (ready) {
      ...
      cross.innerHTML = `<div class="cross-icon">⧉</div><div><strong>Sources prêtes</strong><p>Les trois schémas sont valides. Le croisement peut être lancé.</p></div>`;
    }
  }
  ...
  async function runAnalysis() {
    ...
    try {
      ...
      showModel();
      closeImport();
      AU.ui.toast('Croisement réussi. Analysis Ultimate est prêt.', 'good');
    } catch (err) {
      console.error(err);
      cross.className = 'cross-status bad';
      ...
      btn.textContent = 'Lancer le croisement';
      updateRunButton();
    }
  }
  ...
  function switchView(view) {
    ...
    if (state.model) AU.ui.render(view, state.model, $('#viewRoot'));
  }
  ...
  async function clearLocalData() {
    if (!confirm('Effacer la session Analysis Ultimate et les snapshots de stock enregistrés dans ce navigateur ?')) return;
  ...
  async function init() {
    ...
    if (!window.XLSX || !window.Papa) {
      ...
      $('#fatal').innerHTML = `<strong>Bibliothèque d’import indisponible.</strong><br>Analysis Ultimate charge SheetJS et Papa Parse depuis des versions épinglées. Vérifiez la connexion Internet puis rechargez la page.`;
    }
    await restoreSession();
    if ('serviceWorker' in navigator && location.protocol.startsWith('http')) navigator.serviceWorker.register('sw.js').catch(() => {});
  }

  return { init, openImport, runAnalysis, switchView, state };
})();
