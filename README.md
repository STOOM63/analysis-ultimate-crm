...
## Correctif v1.0.1 — import CSV Firefox

Le moteur d’import Ventes conserve Papa Parse en Web Worker pour les gros CSV, mais la normalisation des en-têtes est désormais effectuée après réception des lignes. Cela évite l’erreur Firefox `Function object could not be cloned.` qui pouvait bloquer l’import d’un fichier `Ventes.csv`. Le cache du Service Worker a également été incrémenté afin de forcer le chargement du correctif.
